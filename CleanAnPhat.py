...
def handle_missing_data_with_KNN(df):
    ...
    # Chọn các cột cần thiết cho quá trình impute
    X = df[['Kích thước', 'Cân nặng']]
    
    # Khởi tạo và huấn luyện mô hình KNNImputer
    imputer = KNNImputer(n_neighbors=300)  # Số láng giềng có thể điều chỉnh
    X_imputed = imputer.fit_transform(X)
    
    # Thay thế các cột 'Kích thước' và 'Cân nặng' trong DataFrame bằng dữ liệu đã được xử lý
    df[['Kích thước', 'Cân nặng']] = X_imputed
